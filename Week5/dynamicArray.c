...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	if (rChild < last)
	{
		if ( compare(dyGet(heap,lChild), dyGet(heap,rChild)) == -1)
		{
			minChild = lChild;
		}
		else { minChild = rChild; }

		if ( compare(dyGet(heap, minChild), dyGet(heap, position)) == -1)
		{
			dySwap(heap, position, minChild);
			adjustHeap(heap, last, minChild, compare);
		}
	}
	else if (lChild < last)
	{
		if (compare(dyGet(heap, lChild), dyGet(heap, position)) == -1)
		{
			dySwap(heap, position, lChild);
			adjustHeap(heap, last, lChild, compare);
		}
	}
...
void buildHeap(DynamicArray* heap, compareFunction compare)
{
	...
	int maxSize = dySize(heap);

	for (int i = maxSize/ 2 - 1; i >= 0; i--)
	{
		adjustHeap(heap, maxSize, i, compare);
	}
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
	...
	int index = dySize(heap) - 1;
	int parent = (index - 1) / 2;
	void* parentValue = heap->data[parent];

	while (index > 0 && compare(value, parentValue) == -1)
	{
			dySwap(heap, index, parent);
			index = parent;
			parent = (index - 1) / 2;
...
{
    // FIXME: implement -- DONE --
	int size = dySize(heap) - 1;
	heap->data[0] = dyGet(heap, size);
	heap->size -= 1;
	adjustHeap(heap, size, 0, compare);
